...
class BBBMeetingModeratorAccessCheck implements AccessCheckInterface {
  ...
  public function access(Route $route, Request $request, AccountInterface $account) {
    ...
    if ($this->account->hasPermission('moderate meetings') || $this->account->hasPermission('administer big blue button') ||
    ($this->account->id() == $node->getAuthor()->id() && $this->account->hasPermission('moderate own meetings'))) {
      return self::ALLOW;
    }
    return self::DENY;
  }
